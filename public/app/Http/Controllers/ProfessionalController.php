...
class ProfessionalController extends Controller
{
    ...
    public function update()
    {
        ...
        if(!$professional){
            ...
            if(!$professional->saveProfessional($dataToSet))
                returnResponse(['message' => translate('professional not updated')], 500);
        }
        if($newProfessional){
    ...
    public function updateDataPerson()
    {
        ...
        $professional = $person->getProfile(Profile::PROFESSIONAL);
        if(!$professional)
            returnResponse(['message' => translate('no professional found'), 400]);
        ...
        if(!$dataPerson->saveDataPerson($data))
            returnResponse(['message' => translate('data person not updated'), 400]);
        returnResponse($dataPerson);
    }
    ...
    public function manageProfessionalJobModality()
    {
        ...
        $professional = $person->getProfile(Profile::PROFESSIONAL);
        if(!$professional)
            returnResponse(['message' => translate('professional not found')], 400);
        $data = null;
    ...
    public function manageProfessionalProfessions()
    {
        $actions = ['add', 'remove', 'list'];
        if(!in_array(request('action'), $actions))
            returnResponse(['message' => 'invalid action'], 400);
        $person = Auth::user();
        $professional = $person->getProfile(Profile::PROFESSIONAL);
        if(!$professional)
            returnResponse(['message' => translate('professional not found')], 400);
    ...
    public function find($professional_slug)
    {
        ...
        returnResponse($professional->gatherInformation());
    }
}
