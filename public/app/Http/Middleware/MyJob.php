...
class MyJob extends Controller
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if(!$jobList)
            Validator::throwResponse(translate('job not found'), 403);
        if(!$this->isException()){
            $person = Auth::user();
            $company = $person->getProfile(Profile::COMPANY);
            if(!$company)
                Validator::throwResponse(translate('company not found'), 403);
            if($jobList->company_id != $company->company_id)
                Validator::throwResponse(translate('not your job'), 403);
        }
